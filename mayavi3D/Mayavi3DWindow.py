...
class Mayavi3idiView(HasTraits):
    ...
    actor = Any()
    actors = List(Actor.Actor)  #A traited list.
    #center = Tuple()
    #point1 = Array()
    #point2 = Array()
    normal = Array()
    ...
    def rebind_picker(self):
        print("Attempting to rebind picker...")
    ...
    def append_actor(self, name, color):
        ...
        a = Actor.Actor(self, self.cur_ActorIndex)
    ...
    def reset_volume_slice(self, length=127, volume_slice=False):
        ...
        print("Slice_edges", self.slice_edges)
        #if self.slice_edges is not None:
        #I resent this try-catch-except block, but it's necessary.
        try:
            self.slice_edges.remove() if self.slice_edges is not None else None
        except ValueError:
            print("slice_edges already removed, passing")
            pass
        self.insert_volume_slice(length=length, volume_slice=volume_slice)
        print("Slice_edges2", self.slice_edges)
